...
function writeLocal(arr: ScoreEntry[]) {
  try {
...
function mergeAndSort(entries: unknown[]): ScoreEntry[] {
...
export function initLeaderboard() {
  ...
  const els = {
    nameInput: document.getElementById('playerName') as HTMLInputElement | null,
    saveNameBtn: document.getElementById('saveNameBtn'),
    list: document.getElementById('scoreList'),
    hint: document.getElementById('highscoreHint'),
  }

  function render(list: ScoreEntry[]) {
    if (!els.list) return
    els.list.innerHTML = ''
    const medals = [
      { emoji: '🥇', color: '#d4af37' },
  ...
  function initName() {
    ...
    els.saveNameBtn?.addEventListener('click', async () => {
      ...
      try {
        ...
        const existing = data.find((e: any) => sanitizeName(e.name).toLowerCase() === v.toLowerCase())
        if (existing) {
    ...
    els.nameInput?.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') els.saveNameBtn?.dispatchEvent(new MouseEvent('click'))
    })
  }

  async function report(_scoreIgnored: number) {
